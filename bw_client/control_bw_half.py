...
def control_bw(bw,path,device):
    ...
    for addr in addr_lst[3:]:
        set_cmd = f'tcset {device} --rate {int(bw/4)}Kbps --network {addr} --direction outgoing --add'
        p = subprocess.Popen(set_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        p.wait()
        set_cmd = f'tcset {device} --rate {int(bw/4)}Kbps --network {addr} --direction incoming --add'
        p = subprocess.Popen(set_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        p.wait()
