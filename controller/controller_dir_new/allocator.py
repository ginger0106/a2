...
class allocator():
    ...
    def process_and_save_data(self,cost,x,y,cost_opt,cost_heu,overhead,continuous_x,hue_x):
        ...
        if self.inter==0:
            ...
            re = {'avg_latency':avg_latency,'avg_bw':avg_bw,'avg_acc':avg_acc,'history':self.history_list,'cost':cost,
                  ...
                  'continuous_x':continuous_x, 'heu_x':hue_x
                  }
    ...
    def RA_rounding(self,x,y,K,I,S,Qt):
        ...
        z_skijqh = self.compute_z(K,I,S,Qt,y,x)
        for k in K:
            for j in range (self.J):
                for i in range(I):
                    for h in self.adaptive_H(self.device_type,k,i):
...
#################
    ...
    def resemble_x(self,S,Qt,k, i,j, h,z_skijqh,x):
        ...
        for s in S:
            sum_z = self.compute_z_in_s (s, Qt, k, i, h, z_skijqh)
    ...
    def compute_z_in_s(self,s,Qt,k, i,h,z_skijqh):
        sum_z = 0
        for q in range (len (Qt[k])):
            for j in range(self.J):
